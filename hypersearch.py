...
from oto_distillation import model_train, choose_model, distill_test
...
def raw_experiment(configs):
    ...
    idx_no_train = torch.LongTensor(np.setdiff1d(np.array(range(len(labels))), idx_train.cpu())).to(configs['device'])
    ...
    cas = load_cascades(cascade_dir, configs['device'],dataset=configs['dataset'], final=False)#加载教师模型的output.txt 默认TRUE
