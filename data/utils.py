...
def load_tensor_data(dataset, labelrate, device):
    #获取邻接矩阵，特征矩阵，独热标签（即真实标签）并划分训练集、验证集、测试集
    adj, features, labels_one_hot, idx_train, idx_val, idx_test = load_dataset_and_split(labelrate, dataset)
    adj = preprocess_adj(adj)#邻接矩阵标准化
    features = preprocess_features(features)#特征矩阵标准化
    adj_sp = adj.tocoo()
    adj = torch.FloatTensor(np.array(adj.todense()))
    features = torch.FloatTensor(np.array(features.todense()))
    labels = labels_one_hot.flatten().astype(int)
