...
def arg_parse(parser):
    parser.add_argument('--dataset', type=str, default='camel-1.2', help='Dataset')
...
def distill_train(all_logits, dur, epoch, model, optimizer, conf, G, labels_init, labels, idx_no_train, idx_train,
    ...
    if conf['model_name'] == 'PLP':
        #train or notrain
        loss = my_loss(logp, cas[-1])#torch.LongTensor(np.eye(2)[labels.tolist()])[idx_train].to(0)/cas[-1][idx_no_train]
    else:
...
def distill_test(conf, model, G, labels_init, labels, idx_test, cas):
    ...
    acc_test = accuracy(logp, labels)#[idx_test]
